...
public class EmailService(IConfiguration configuration, ILogger<EmailService> logger) : IEmailService
{
    ...
    public async Task SendPasswordResetEmailAsync(string to, string fullName, string rawToken)
    {
        var apiKey = configuration["SENDGRID_API_KEY"]
                     ?? throw new InvalidOperationException("SENDGRID_API_KEY not configured");
        var fromEmail = configuration["SENDGRID_FROM_EMAIL"] ?? "noreply@eduspace.app";
        ...
        if (string.IsNullOrWhiteSpace(frontendBaseUrl))
            throw new InvalidOperationException(
                "FRONTEND_BASE_URL no está configurado. No se puede generar el enlace de recuperación.");
        ...
        var htmlBody = $"""
            ...
            <body style="font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 20px;">
              <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; padding: 32px;">
                <h2 style="color: #1a1a1a;">¡Hola, {fullName}!</h2>
                <p style="color: #444; font-size: 16px;">
                  Recibimos una solicitud para restablecer la contraseña de tu cuenta de <strong>EduSpace</strong>.
                ...
                <p style="color: #999; font-size: 13px;">
                  Este enlace es válido por <strong>1 hora</strong>. Si no solicitaste restablecer tu contraseña, ignorá este correo.
                </p>
              </div>
            </body>
            </html>
        ...
        var toAddr  = new EmailAddress(to);
        var msg     = MailHelper.CreateSingleEmail(from, toAddr,
            subject: "Restablecé tu contraseña de EduSpace",
            plainTextContent: $"Hola {fullName},\n\nRestablecé tu contraseña haciendo clic en: {resetLink}\n\nEste enlace vence en 1 hora.",
            htmlContent: htmlBody);
        ...
        {
            var error = await response.Body.ReadAsStringAsync();
            logger.LogError("SendGrid password-reset email failed: {Status} {Body}", response.StatusCode, error);
            throw new InvalidOperationException($"SendGrid failed: {response.StatusCode}");
        }

        logger.LogInformation("Password-reset email enviado a {To} via SendGrid", to);
    }
}
